...
def generate_template_preserving_presentation(
    reference_path: Path,
    ...
    if _can_populate_reference_deck(presentation):
...
def _populate_reference_deck(presentation: Presentation, analysis: PresentationAnalysis) -> None:
    slide_payloads = _build_template_slide_payloads(analysis)
    slides = list(presentation.slides)
    slides_to_use = min(len(slides), len(slide_payloads))

    for slide_index in range(slides_to_use):
        _replace_placeholder_shapes(slides[slide_index], slide_payloads[slide_index])

    if len(slides) > slides_to_use:
        _trim_slides_after(presentation, slides_to_use)


def _build_template_slide_payloads(analysis: PresentationAnalysis) -> list[list[str]]:
    summary_metrics = [
        f"Campaigns: {analysis.kpis.get('campaign_count', '-')}",
        f"Revenue: {analysis.kpis.get('total_revenue', '-')}",
        f"Investment: {analysis.kpis.get('total_investment', '-')}",
        f"Average ROI: {analysis.kpis.get('average_roi', '-')}",
    ]
    compact_summary = " | ".join(summary_metrics)
    ...
    payloads = [
        ...
        [
            ...
            "Insight 2",
            _nth_or_default(analysis.key_insights, 1, _first_or_default(analysis.trends, "Performance varies materially across channels.")),
            "Insight 3",
            _nth_or_default(analysis.key_insights, 2, "Top campaigns and regions drive the current outcome profile."),
        ],
        [
            "Trends and Risks",
            "Trend 1",
            _first_or_default(analysis.trends, "Efficiency patterns point to a few strong performing segments."),
            "Trend 2",
            _nth_or_default(analysis.trends, 1, "Returns vary across regions and channels."),
            "Risk 1",
            _first_or_default(analysis.risks, "Performance concentration can increase exposure to fewer segments."),
        ],
    ]

    campaign_payload = _build_top_items_payload(
        title="Top Campaigns",
        intro="Campaign performance",
        rows=analysis.top_campaign_rows,
    )
    if campaign_payload:
        payloads.append(campaign_payload)

    segment_payload = _build_top_items_payload(
        title="Leading Segments",
        intro="Region and channel leaders",
        rows=(analysis.region_rows + analysis.channel_rows),
    )
    if segment_payload:
        payloads.append(segment_payload)

    impact_payload = _build_impact_payload(analysis)
    if impact_payload:
...
def _build_top_items_payload(title: str, intro: str, rows) -> list[str]:
    if not rows:
        return []
...
def _trim_slides_after(presentation: Presentation, keep_count: int) -> None:
    slide_ids = list(presentation.slides._sldIdLst)
